...
# -*- coding: utf-8 -*- #
# ------------------------------------------------------------------
# @File Name:        spider_all.py
# @Author:           wen
# @Version:          ver0_1
# @Created:          2021/1/23 8:46
...
if __name__ == '__main__':
    total_dir = Path(r"E:\PycharmProjects\my_api_02\spider_beauty\tujigu")
    i = 0
    for single_dirpath in total_dir.iterdir():
        print(single_dirpath)
        single_dirname = single_dirpath.name
        print(single_dirname)
        name, ids = str(single_dirname).split("_")
        print(name, ids)
        tlist = []
        t = Process(target=spider_image, args=(name, ids))
        tlist.append(t)
        i += 1
        t.start()
        if i % 10 == 0:
